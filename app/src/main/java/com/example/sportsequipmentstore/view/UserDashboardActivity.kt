...
import com.example.sportsequipmentstore.view.AddressActivity
import com.example.sportsequipmentstore.view.CartActivity
import com.example.sportsequipmentstore.view.EditProfileActivity
import com.example.sportsequipmentstore.view.WishlistActivity
...
class UserDashboardActivity : ComponentActivity() {
    ...
    override fun onResume() {
        super.onResume()
        // Refresh user data and orders whenever activity resumes (i.e., after EditProfile)
        val currentUserId = userViewModel.getCurrentUser()?.uid
        currentUserId?.let {
            userViewModel.getUserById(it)
            orderViewModel.loadOrdersByUser(it)
...
fun UserDashboardBody(
    ...
    // Whenever currentUserId changes or on start, fetch user and orders
    LaunchedEffect(currentUserId) {
        currentUserId?.let {
            userViewModel.getUserById(it)
            orderViewModel.loadOrdersByUser(it)
    ...
    Scaffold(
        ...
        Column(
            ...
            // Header with profile picture and name - always reflects latest user info
            UserHeader(user)
    ...
    Row(
        ...
        if (!user?.image.isNullOrEmpty()) {
            AsyncImage(
                model = ImageRequest.Builder(LocalContext.current)
                    .data(user?.image)
                    .diskCachePolicy(CachePolicy.DISABLED) // Disable cache so changes show immediately
                    .memoryCachePolicy(CachePolicy.DISABLED)
                    .build(),
                contentDescription = "Profile Picture",
                modifier = imageModifier,
